    ...
    <AppWrapper>
      <Grid className="centered pt-4" container spacing={2}>
        ...
        <Grid item xs={12}>
          ...
          <FormControl>
            <FormLabel id="demo-controlled-radio-buttons-group">{languageMap.splash.languageSelect}</FormLabel>
            <RadioGroup
              row
              ...
              onChange={async (e: any) => {
                ...
                // 获取语言字典
                await window.sideAPI.system.getLanguageMap().then(setLanguageMap);
              }}
            >
              <FormControlLabel value="en" control={<Radio />}
                                label={language === "en" ? "english" : "英文"} />
              <FormControlLabel value="cn"
                                control={<Radio />}
