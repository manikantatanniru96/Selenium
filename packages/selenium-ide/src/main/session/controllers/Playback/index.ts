...
export default class PlaybackController extends BaseController {
  ...
  async getPlayback(testID?: string, forceNewPlayback = false) {
    console.info("项目列表:" + JSON.stringify(this.session.projects.project));
    console.info("用例列表:" + JSON.stringify(this.session.projects.project.tests));
    const browserInfo = this.session.store.get("browserInfo");
    console.info("获取browserInfo:" + JSON.stringify(browserInfo));
    const makeNewPlayback = !this.playbacks.length || forceNewPlayback;
    let playback: Playback;
    if (makeNewPlayback) {
      console.info("是否新建Play对象:" + makeNewPlayback);
  ...
  async openBrowser(url: string) {
    const browserInfo = this.session.store.get("browserInfo");
    const executor = await this.session.driver.build({
      browser: browserInfo.browser
    });
    await executor.driver.get(url);
    await executor.driver.manage().window().maximize();
  }
  ...
  async play(
    ...
    if (playRange[1] !== -1) {
      ...
      console.info("执行测试用例:" + JSON.stringify(this.session.tests.getByID(testID)));
